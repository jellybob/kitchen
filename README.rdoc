...
* to install from source: a working git framework (for example provided by Opscode git cookbook)

Tested on ubuntu. Should work on other system with upstart.

= ATTRIBUTES:

* node/install_from_source : If set to true, installs node from git, otherwise uses packages.
